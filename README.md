...
구조화 도구는 `source` 필드로 "어느 파일의 언제 상태에서 나온 사실인지"도 함께 돌려줌 — 데이터 갱신일이 곧 버전임.
...
MCP 프롬프트도 3개 제공함.
`candidate_briefing`은 채용 담당자용 브리핑을, `tech_deep_dive`는 주제별 기술 딥다이브를,
`job_fit`은 채용 공고 요건을 한 줄씩 근거와 대조해 표로 정리하게 함(판정은 충족·부분·근거 없음 셋뿐이고,
`found=false` 를 "비슷한 경험이 있다"로 바꾸지 말라고 못 박아 둠).
...
이 평가는 `python eval_search.py`로 재현됨(현재는 질의 14문항에 top1 14/14. 개선 8·9 반영).
...
문턱을 '코퍼스에 있는 토큰 수'로만 계산해 봤더니 자연어 질의는 살아났지만 무의미 거부가 6/6에서 3/6으로 무너짐.
점수 축에서는 두 집합이 겹쳐서 임계값을 어디에 둬도 한쪽이 깨짐.

| 질의 | 최고점 / 토큰수 | |
|---|---|---|
| `vLLM 써봤어요?` | 1.1 / 5 = 0.23 | 정상 |
| `블록체인 스마트컨트랙트 가스비` | 5.3 / 14 = 0.38 | 무의미 |
| `양자컴퓨팅 큐비트 결맞음` | 5.1 / 11 = 0.47 | 무의미 |
| `추론 최적화 어떻게 했어요` | 10.6 / 11 = 0.97 | 정상 |

갈리는 축은 점수가 아니라 어휘였음.
무의미 질의는 온전한 단어(bigram으로 만든 조각 말고)가 코퍼스에 하나도 없고(0/2~0/4), 정상 질의는 자연어라도 반드시 하나는 있음.
그래서 질의에 코퍼스 단어가 있으면 점수 문턱을 걷음.

관문만 쓰면 `쿠버네티스로 뭐 했어`가 탈락함. 조사가 붙어 온전한 단어가 코퍼스에 없기 때문.
그래서 둘의 합집합으로 둠. 점수 문턱을 넘거나, 코퍼스 단어를 담고 있으면 통과.

평가 질의에 자연어 2문항을 추가해 12문항에서 14문항이 됐고, top1 14/14·무의미 거부 6/6.
관문을 되돌리면 평가가 exit 1로 떨어지는 것까지 확인함.
...
재는 것과 안 재는 것을 갈라 뒀음.
도구가 답에 필요한 사실을 내놓는지는 서버 책임이라 여기서 재고,
그 응답으로 LLM 이 옳은 문장을 만드는지는 클라이언트·모델 몫이라 재지 않음.

채점기부터 의심했음.
초판은 구분자를 지운 문자열 포함으로 채점했는데 정답 `8` 이 응답의 `2018` 에 걸려 통과했음.
지금은 숫자 앞뒤 자릿수 경계를 요구하고, 날짜·범위 구분자만 흔들림을 허용함
(`2023-05-25` 는 인정, `2023.05.31` 은 불통과 — 실제로 시험해 확인).
