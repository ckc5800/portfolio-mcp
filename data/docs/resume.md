...
▸ 운영 고도화
vLLM(GPU)과 Supertonic(CPU) 듀얼 엔진 구성. 무중단 롤링 재배포 체계를 만들고, 온보딩 가이드·운영 런북·장애복구 절차·SLO까지 문서화해 운영 이관이 가능한 수준으로 정비.
▸ 성과 (KPI)
✓ TTFB: 2292ms → 334ms (85% 단축, 6.85배 개선)
✓ Throughput: 자체 튜닝(FP8·executor)으로 0.68 → 1.41 rps, 엔진 이전 후 최대 안정치 3.09 rps
✓ 동시 스트림 수용 한계 36 실측 (2026.07 말) | burst p95 TTFB 1,008ms → 652ms (동시 20 기준)
✓ 단건 지연시간 15% 감소 | RTF 0.1 이하 | CER(문자 오류율) 3~5%
✓ 32개 언어 지원 단일 플랫폼 | ITN 정규화 정확도 50.1% → 70.9%
✓ 이후 고도화(vLLM 마이그레이션·FP8): TTFB p50 70ms, 처리량 8.6 rps — 콜센터 활성 400명 규모 여유 (2026.07 실측)
▸ 핵심 엔지니어링 (6가지 문제 해결)
...
⑥ SSE 스트리밍 호환: 첫 청크에만 WAV 헤더가 붙어 브라우저 오디오 디코딩이 이후 청크에서 실패하던 문제를 분석해, 청크별 헤더 수정과 실시간 SSE 엔드포인트 신설로 해결
▸ 기술 스택
...
• 화자 분리 결과를 STT에 결합하는 하이브리드 화자 할당 로직을 구현 — 경계가 모호한 발화는 Ambiguous로 분리해 화자 오할당을 방지
